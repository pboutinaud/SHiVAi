...
RUN apt-get update && apt-get install -y --no-install-recommends \
    build-essential libcairo2 libgdk-pixbuf2.0-0 graphviz wget unzip dos2unix weasyprint \
    libpango-1.0-0 libpangoft2-1.0-0 libffi-dev libjpeg-dev libopenjp2-7-dev && \
    rm -rf /var/lib/apt/lists/*
# RUN apt-get update && apt-get install -y --no-install-recommends libharfbuzz-subset0 libharfbuzz0b && rm -rf /var/lib/apt/lists/*
