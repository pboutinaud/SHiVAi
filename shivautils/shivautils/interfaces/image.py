...
class ConformOutputSpec(TraitedSpec):
    ...
    Args:
        conform (nb.Nifti1Image): file img IRM nifti transformed
    """
    resampled = traits.File(exists=True,
                            desc='img conform to desired voxel and dimension')
...
class IntensityNormalizationInputSpec(BaseInterfaceInputSpec):
    ...
    image"""
    input_image = traits.File(exists=True, desc='file img Nifti',
                              mandatory=True)
...
class Normalization(BaseInterface):
    ...
    Attributes:
        input_spec (nb.Nifti1Image): file img Nifti
        output_spec (nb.Nifti1Image): file img IRM nifti transformed
...
class Apply_maskInputSpec(BaseInterfaceInputSpec):
    """Input parameter to apply an treshold function on
    nifti image"""
...
class Apply_maskOutputSpec(TraitedSpec):
    ...
    Args:
        brain_mask (nb.Nifti1Image): brain_mask of nifti image given
    """
    brain_mask = traits.File(exists=True,
                             desc='brain_mask of nifti image given')
