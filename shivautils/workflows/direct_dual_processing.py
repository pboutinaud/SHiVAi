...
def genWorkflow(**kwargs) -> Workflow:
    ...
    Returns:
        workflow
    """
    name_workflow = "test_processing_bind_histogram"
    ...
    save_bounding_crop = Node(Function(input_names=['img_apply_to',
                                                    'bbox1',
                                                    'bbox2',
                                                    'cdg_ijk',
                                                    'brainmask'],
                             output_names=['bounding_crop'], function=bounding_crop), name='save_bounding_crop')
    
    workflow.connect(conform, 'resampled', save_bounding_crop, 'img_apply_to')
    workflow.connect(hard_brain_mask, 'thresholded', save_bounding_crop, 'brainmask')
    workflow.connect(crop, 'bbox1', save_bounding_crop, 'bbox1')
    workflow.connect(crop, 'bbox2', save_bounding_crop, 'bbox2')
    workflow.connect(crop, 'cdg_ijk', save_bounding_crop, 'cdg_ijk')
    ...
    workflow.connect(crop, 'cropped', qc_overlay_brainmask, 'img_ref')
    ...
    if kwargs['SYNTHSEG']:
        ...
        workflow.connect(predict_wmh, 'segmentation', wmh_quantification_latventricles, 'wmh')
        ...
        workflow.connect(predict_pvs, 'segmentation', pvs_quantification_bg, "img")
    ...
    metrics_predictions_pvs.inputs.pvs = True
    ...
    summary_report.inputs.sum_workflow = os.path.join(kwargs['BASE_DIR'], name_workflow, 'graph.svg')
    ...
    workflow.connect(save_bounding_crop, 'bounding_crop', summary_report, 'img_bounding_crop')
    ...
    if kwargs['SYNTHSEG']:
        workflow.connect(wmh_quantification_latventricles, 'csv_clusters_localization', datasink, 'csv_clusters_localization')
        workflow.connect(pvs_quantification_bg, 'metrics_bg_pvs', datasink, 'pvs_quantification_bg')
    ...
    workflow.connect(subject_list, 'subject_id', metrics_predictions_pvs_generale, 'subject_id')
    ...
    workflow.connect(subject_list, 'subject_id', metrics_predictions_wmh_generale, 'subject_id')
