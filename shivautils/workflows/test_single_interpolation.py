...
def genWorkflow(**kwargs) -> Workflow:
    ...
    Returns:
        workflow
    """
    workflow = Workflow("single_T1_testsetglobal_V1_PVS")
    ...
    predict_gin = Node(Predict(), name="predict_gin")
    predict_gin.plugin_args = {'sbatch_args': '--nodes 1 --cpus-per-task 4 --gpus 1'}
    predict_gin.inputs.snglrt_bind =  [
    ...
    predict_gin.inputs.model = '/mnt/model'
    predict_gin.inputs.snglrt_enable_nvidia = True
    predict_gin.inputs.descriptor = kwargs['PVS_DESCRIPTOR']
    predict_gin.inputs.snglrt_image = '/bigdata/yrio/singularity/predict_2.sif'
    predict_gin.inputs.out_filename = 'gin_pvs_map.nii.gz'

    workflow.connect(datagrabber, "GIN", predict_gin, 't1')
    ...
    predict_fealinx = Node(Predict(), name="predict_fealinx")
    predict_fealinx.plugin_args = {'sbatch_args': '--nodes 1 --cpus-per-task 4 --gpus 1'}
    predict_fealinx.inputs.snglrt_bind =  [
        (kwargs['BASE_DIR'],kwargs['BASE_DIR'],'rw'),
        ('`pwd`','/mnt/data','rw'),
        ('/bigdata/resources/cudas/cuda-11.2','/mnt/cuda','ro'),
        ('/bigdata/resources/gcc-10.1.0', '/mnt/gcc', 'ro'),
        (kwargs['MODELS_PATH'], '/mnt/model', 'ro')]
    predict_fealinx.inputs.model = '/mnt/model'
    predict_fealinx.inputs.snglrt_enable_nvidia = True
    predict_fealinx.inputs.descriptor = kwargs['PVS_DESCRIPTOR']
    predict_fealinx.inputs.snglrt_image = '/bigdata/yrio/singularity/predict_2.sif'
    predict_fealinx.inputs.out_filename = 'fealinx_pvs_map.nii.gz'

    workflow.connect(final_norm, "intensity_normalized", predict_fealinx, 't1')
