...
def create_edges(path_image, path_ref_image, path_brainmask, nb_of_slices=8):
    ...
    for i in range(3):
        ...
        for j in np.linspace(65,image.shape[i]-66, nb_of_slices):
    ...
    figure, axis = plt.subplots(3, nb_of_slices, figsize=(3*nb_of_slices-4, nb_of_slices)) 
    figure.patch.set_facecolor('k')
    
    for i in range(nb_of_slices):
         
        axis[0, i].imshow(
        ndimage.rotate(sli_ref_image[i],90)[..., 0],
        cmap='gray')

        axis[0, i].imshow(
        ndimage.rotate(sli_image[i],90)[..., 0],
        cmap=my_cmap)

        axis[0, i].set_xlabel('x = ' + str(X_Y_Z[i]))
        axis[0, i].xaxis.label.set_color('white')
        axis[0, i].get_xaxis().set_ticks([])
        axis[0, i].get_yaxis().set_ticks([])            
        
        axis[1, i].imshow(
        ndimage.rotate(sli_ref_image[i+nb_of_slices],90)[..., 0],
        cmap='gray')
        axis[1, i].imshow(
        ndimage.rotate(sli_image[i+nb_of_slices],90)[..., 0],
        cmap=my_cmap)           
        
        axis[1, i].set_xlabel('y = ' + str(X_Y_Z[i]))
        axis[1, i].xaxis.label.set_color('white')
        axis[1, i].get_xaxis().set_ticks([])
        axis[1, i].get_yaxis().set_ticks([])
            
        axis[2, i].imshow(
        ndimage.rotate(sli_ref_image[i+2*nb_of_slices],90)[..., 0],
        cmap='gray')           
        axis[2, i].imshow(
            ndimage.rotate(sli_image[i+2*nb_of_slices],90)[..., 0], 
            cmap=my_cmap)
                
        axis[2, i].set_xlabel('z = ' + str(X_Y_Z[i]))
        axis[2, i].get_xaxis().set_ticks([])
        axis[2, i].xaxis.label.set_color('white')
...
path_image = "/homes_unix/yrio/Documents/data/Set_re_entrainement_Unet/single_T1_testset_preprocessing/_subject_id_SHARE0055/hard_post_brain_mask/brain_mask_map_thresholded.nii.gz"
path_ref_image = "/homes_unix/yrio/Documents/data/Set_re_entrainement_Unet/single_T1_testset_preprocessing/_subject_id_SHARE0055/_interpolation_HammingWindowedSinc/final_intensity_normalization/SHARE0055_T1_raw_trans_img_normalized.nii.gz"
path_brainmask = "/homes_unix/yrio/Documents/data/Set_re_entrainement_Unet/single_T1_testset_preprocessing/_subject_id_SHARE0055/hard_post_brain_mask/brain_mask_map_thresholded.nii.gz"

create_edges(path_image, path_ref_image, path_brainmask, nb_of_slices=8)