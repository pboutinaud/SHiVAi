...
class MakeDistanceVentricleMapsInputSpec(CommandLineInputSpec):
    
    #os.system("niimath ventricle_mask  -binv -edt n132")
    im_file = traits.Str(mandatory=True,
                            desc ='Ventricle masks (binirized)',
                            argstr='%s',
                            position=1)
    
    out_file = traits.Str('distance_map.nii.gz',
                           mandatory=True,
                           desc='Output Distance Ventricle maps',
                           argstr='%s',
                           position=2)
    
    
class MakeDistanceVentricleMapsOutputSpec(TraitedSpec):
    out_file = File(exists=True)


class MakeDistanceVentricleMap(CommandLine):
    """
    ...
    _cmd = op.join(p, 'MakeDistanceVentricleMap.sh')
    input_spec =  MakeDistanceVentricleMapsInputSpec
    output_spec = MakeDistanceVentricleMapsOutputSpec
