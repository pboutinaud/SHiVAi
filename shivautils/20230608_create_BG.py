...
def createBGsliceMask(path_to_freesurfer_segm, out_fname_root=''):
    ...
    segm_im = nib.load(path_to_freesurfer_segm)
    segm_dat = segm_im.get_fdata()
    ...
    bg_roi_nii = nib.Nifti1Image(bg_box, segm_im.affine, segm_im.header)
    out_fname = '{}_BG_slice.nii.gz'.format(out_fname_root) if out_fname_root else 'BG_slice.nii.gz'
    nib.save(bg_roi_nii, out_fname)

    return(op.abspath(out_fname))

path_to_freesurfer_segm = '/homes_unix/yrio/Documents/data/TestSetGlobal/PVS_WMH/T1_raw/sub-101/main/sub-101_space-T1_desc-masked_T1.nii.gz'
createBGsliceMask(path_to_freesurfer_segm, out_fname_root='/homes_unix/yrio/Documents')
                
#BGsliceMask = Node(Function(input_names=['path_to_freesurfer_segm'],
                #output_names='out_fname',
                #function=createBGsliceMask),
        #name='BGsliceMask')

# example of connecting the freesurfer segmentation image to the createBGsliceMask function
#wf.connect(scanList,'path_to_freesurfer_segm', BGsliceMask, 'path_to_freesurfer_segm')
