...
def make_report(img_normalized,
    ...
    tm = Template(
            """<!DOCTYPE html>
                ...
                <div class="table">
                    <h2>Prediction results for PeriVascular Spaces (PVS)</h2>
                    <table>
                        <tr class="row">
                            {% for col in columns %}
                            <th class ="cell">{{ col }}</th>
                ...
                <div class="table">
                    ...
                    <h2>Predictions results clusters PVS in Basal Ganglia and DWMH</h2>
                    <table>
                        <tr class="row">
                    ...
                    <p>Clusters Filter : {{ clusters_filter_bg }}</p>
                    {% endif %}
                </div>
                <div class="test">
                    <h1>Quality control</h1>
                    <h2>Preprocessed T1w image histogram</h2>
                    <p>Histogram of the T1w image that enters the classifier: {{ resolution }} mm<sup>3</sup> with {{ image_size }} shape, (within brain mask and with censoring for voxels outside the brain mask)</p>
                    <img src = {{ hist_intensity }} width="400" height="400"></img>
                    <h2>Crop box</h2>Display of the cropping region on the conformed image (256x256x256 at 1.0 mm<sup>3  resolution).</p>
                    <img src = {{ bounding_crop }}></img>
                    ...
                    <h2>Overlay of final brainmask over cropped T1w</h2>
                    <p>Overlay of the brainmask on T1w image : {{ resolution }} mm<sup>3</sup> with {{ image_size }}, (with censoring for voxels outside the brain mask)</p>
