...
out_dir = args.output
wfargs_1 = {'SUBJECT_LIST': subject_list,
...
wf_preprocessing = genWorkflow(**wfargs_1)
...
wf_predict = genWorkflowPredict(**wfargs_2)
wf_post = genWorkflowPost(**wfargs_3)
...
wf_preprocessing.get_node('conform').inputs.voxel_size = tuple(args.voxel_size)
...
wf_post.get_node('dataGrabber').inputs.template_args = {'segmentation_pvs': [['subject_id']],
                                                        ...
                                                        'CDG_IJK': [['subject_id']],
                                                        'sum_preproc_wf': [[]]}
wf_post.get_node('dataGrabber').inputs.field_template = {'segmentation_pvs': '_subject_id_%s/predict_pvs/map.nii.gz',
                                                         'segmentation_wmh': '_subject_id_%s/predict_wmh/map.nii.gz',
