...
def build_args_parser():
    ...
    parser.add_argument('--threshold', type=float, default=0.4,
                        help='Treshold for the first brain_mask')
...
def main():
    ...
    model = tf.keras.models.load_model(args.model_brainmask, compile=False)
    ...
    thresholded = threshold(img_normalized,
                            thr=args.threshold,
                            binarize=True)
    cropped, cdg_ijk, bbox1, bbox2 = crop(roi_mask=thresholded,
    					  apply_to=img_normalized,
                   			  dimensions=final_dimensions)

    brain_mask = apply_mask(cropped, model)

    cropped_initial_intensities, cdg_ijk, bbox1, bbox2 = crop(roi_mask=thresholded,
    							      apply_to=resampled,
    							      dimensions=final_dimensions,
    							      cdg_ijk=cdg_ijk)
    
    img_final, report_2, mode_2 = normalization(cropped_initial_intensities, 99, brain_mask)

    # save the image to the desired path
    nb.loadsave.save(img_final,
                     args.output)
