...
Apptainer is a container solution, meaning that you can run SHiVAi using an Apptainer image (a file with the *.sif* extension) containing all the software and environment necessary. You can get the SHiVAi image from us [https://cloud.efixia.com/sharing/ga270luKd](https://cloud.efixia.com/sharing/ga270luKd) or build it yourself.
...
Downloading the folder should give you a .zip file (named *SynthSeg_models.zip*).
Put it in the same folder as the `apptainer_synthseg_tf.recipe` file.

> To check if everything is as it should: When unzipped, `SynthSeg_models.zip` should yield a `SynthSeg_models` folder containing the Synthseg models as .h5 files.
