...
def genWorkflow(**kwargs) -> Workflow:
    ...
    field_tpl = {'brainmask': os.path.join(kwargs['BASE_DIR'], preprocDir, '_subject_id_%s/hard_post_brain_mask/post_brain_mask_thresholded.nii.gz'),
    ...
    if 'PVS' in kwargs['PREDICTION'] or 'PVS2' in kwargs['PREDICTION']:
        ofields += ['segmentation_pvs']
        tpl_args.update({'segmentation_pvs': [['subject_id']]})
        field_tpl.update({'segmentation_pvs': '_subject_id_%s/predict_pvs/pvs_map.nii.gz'})
    if 'WMH' in kwargs['PREDICTION']:
        ofields += ['segmentation_wmh']
        tpl_args.update({'segmentation_wmh': [['subject_id']]})
        field_tpl.update({'segmentation_wmh': '_subject_id_%s/predict_wmh/wmh_map.nii.gz'})
    if 'PVS2' in kwargs['PREDICTION'] or 'WMH' in kwargs['PREDICTION']:
        ofields += ['FLAIR_cropped']
        tpl_args.update({'FLAIR_cropped': [['subject_id']]})
        field_tpl.update({'FLAIR_cropped': os.path.join(
            kwargs['BASE_DIR'],
            preprocDir,
            '_subject_id_%s/flair_final_intensity_normalization/t1_to_flair__Warped_img_normalized.nii.gz')})
