...
def main():
    ...
    wf_preprocessing.get_node('dataGrabber').inputs.template_args = {'t1': [['subject_id', 't1']],
    ...
    wf_post.get_node('dataGrabber').inputs.field_template = {'segmentation_pvs': '_subject_id_%s/predict_pvs/pvs_map.nii.gz',
                                                             ...
                                                             'T1_cropped': os.path.join(out_dir, wf_preprocessing.name, '_subject_id_%s/t1_final_intensity_normalization/%s_T1_raw_trans_img_normalized.nii.gz'),
                                                             'FLAIR_cropped': os.path.join(out_dir, wf_preprocessing.name, '_subject_id_%s/flair_final_intensity_normalization/t1_to_flair__Warped_img_normalized.nii.gz'),
