...
def shiva(in_dir, out_dir, input_type, file_type, sub_list, prediction, model, brain_seg, ss_qc, ss_vol, ai_threads, batch_size,
          ...
          swomed_swi, use_t1, container_image, synthseg_image, containerized_nodes, containerized_nodes_docker,
          docker_image, synthseg_docker_image, local_synthseg, prereg_flair,
    ...
    wfargs = {
        ...
        **descriptor_paths,
        ...
        'CONTAINER_IMAGE': docker_image if containerized_nodes_docker else container_image,
        'SYNTHSEG_IMAGE': synthseg_docker_image if containerized_nodes_docker else synthseg_image,
        'CONTAINER_RUNTIME': 'docker' if containerized_nodes_docker else ('singularity' if containerized_nodes else None),
