...
from shivai.interfaces.shiva import PredictDirect
...
def genWorkflow(**kwargs) -> Workflow:
   ...
   # write original image into t1 crop space
    ...
    t1_to_mask.inputs.interpolation = 'WelchWindowedSinc'
    ...
    workflow.connect(t1_norm, "intensity_normalized", predict_wmh, "t1")
    workflow.connect(flair_norm, "intensity_normalized", predict_wmh, "flair")
    ...
    workflow.connect(t1_norm, "intensity_normalized", predict_pvs, "t1")
    workflow.connect(flair_norm, "intensity_normalized", predict_pvs, "flair")
    ...
    workflow.connect(crop_to_t1, 'forward_transforms',
    ...
    workflow.connect(t1_norm, 'intensity_normalized', qc_overlay_brainmask, 'img_ref')
    ...
    workflow.connect(t1_norm, 'report', datasink, 'report_t1_final_normalization')
    ...
    workflow.connect(t1_norm, 'intensity_normalized', datasink, 'T1_cropped_normalize')
    workflow.connect(flair_norm, 'intensity_normalized', datasink, 'FLAIR_cropped_normalize')
