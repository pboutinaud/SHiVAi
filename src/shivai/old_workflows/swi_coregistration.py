...
from shivai.interfaces.image import Normalization, Crop, Conform
...
def genWorkflow(**kwargs) -> Workflow:
    ...
    Returns:
        workflow
    """
    workflow = Workflow("shiva_coregister_normalize_swi")
    ...
    dicom2nifti_source.inputs.args = "-4 N"
    ...
    dicom2nifti_source.inputs.reorient_and_crop = False
    dicom2nifti_source.inputs.source_in_filename = False

    workflow.connect(datagrabber, "source", dicom2nifti_source, "source_names")

    avg = Node(AverageImages(), name="swi_average")
    avg.inputs.dimension = 3
    avg.inputs.output_average_image = "swi_average.nii.gz"
    avg.inputs.normalize = True

    workflow.connect(dicom2nifti_source, 'converted_files',
                     avg, 'images')
    ...
    coreg.inputs.sampling_percentage = [1.0]
    coreg.inputs.output_transform_prefix = "source_to_cropped_t1_"
    ...
    workflow.connect(avg, 'output_average_image',
                     coreg, 'moving_image')
    ...
    workflow.connect(avg, 'output_average_image', conform, 'img')
    ...
    # crop t1 centered on xyz origin (affine matrix)
    ...
    workflow.connect([
        (avg, deface_source, [('output_average_image', 'in_file')]),
